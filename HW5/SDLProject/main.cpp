...
void Initialize() {
    ...
    glClearColor(0.54f, 0.92f, 1.0f, 1.0f);
    glEnable(GL_BLEND);
...
int main(int argc, char* argv[]) {
    ...
    while (gameIsRunning) {
        ...
        if (currentScene->endOfGame == false) {
            Update();
        
            
        if (currentScene->state.nextScene >= 0 && currentScene->state.nextScene < 4) {
            sceneList[currentScene->state.nextScene]->live = currentScene->live;
